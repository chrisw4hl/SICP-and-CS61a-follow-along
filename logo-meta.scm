  ...
  (define (eval-helper paren-flag)
    (let ((token (ask line-obj 'next)))
      (cond ((self-evaluating? token) token)
            ...
            ((make? token) (make line-obj env))
            (else
              (let ((proc (lookup-procedure token)))
                (if (not proc) (error "I don't know how  to " token))
                (cond ((and (list? (arg-count proc)) (< (car (arg-count proc)) 0) paren-flag)
                       (logo-apply proc
                      ...
                      ((list? (arg-count proc))
                       (logo-apply proc
                                   (cons env (collect-n-args (car (arg-count proc))
                                                         line-obj
                                                         env))))
                      ((and (< (arg-count proc) 0) paren-flag)
                       (logo-apply proc
